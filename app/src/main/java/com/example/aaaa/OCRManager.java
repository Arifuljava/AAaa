...
public class OCRManager {
    ...
    public List<String> performOCR(InputImage image) {
        TextRecognizer recognizer = TextRecognition.getClient(TextRecognizerOptions.DEFAULT_OPTIONS);

        recognizer.process(image)
                .addOnSuccessListener(visionText -> processTextBlocks(visionText))
    ...
    private void processTextBlocks(Text visionText) {
        ...
        //Log.d(TAG, "Final detected text list: " + detectedTextList);
        //Log.d(TAG, "Number of detected text: " + detectedTextList.size());
    }
    ...
    private List<List<String>> groupTextByColumns(List<TextObservation> observations) {
        ...
        for (TextObservation observation : observations) {
            ...
            if (previousX == -1 || Math.abs(currentX - previousX) < 20) { // Adjust threshold as needed
